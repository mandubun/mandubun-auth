(()=>{var o={API_BASE_URL:"https://api.mandubun.com",currentUserId:null,currentUserEmail:null,currentUserIsAdmin:!1,isLoggedIn:!1,_refreshingToken:null,userXP:0,userLevel:1,userStreak:0,dailyWordsPracticed:0,DAILY_WORDS_GOAL:10,_cachedDueCount:0,_cachedVocabDue:0,_cachedGrammarDue:0,_cachedOverviewData:null,userIsPro:!1,TTS_READY:!1,TTS_VOICES:[],TTS_SELECTED_VOICE:null,TTS_IS_SPEAKING:!1,TTS_SPEED:"medium",TTS_LAST_REQUEST:0,SPEECH_READY:!1,SPEECH_SUPPORTED:!1,SPEECH_IS_LISTENING:!1,currentMode:"browse",practiceDifficulty:"beginner",practiceFocus:"all",blendedPracticeModeActive:!1,activeBlendedPanelHash:null,blendedSession:null,currentWebtoonId:null,currentEpisodeId:null,isOnline:navigator.onLine,currentPage:"home"};var q="https://jwlylgrtaevgjujstvvg.supabase.co",me="sb_publishable_is4EfIvydGz5COHFX6XKrA_h39lvOZv";function pe(){try{return{accessToken:localStorage.getItem("auth_token"),refreshToken:localStorage.getItem("refresh_token"),expiresAt:parseInt(localStorage.getItem("expires_at")||"0",10),email:localStorage.getItem("user_email")}}catch{return{accessToken:null,refreshToken:null,expiresAt:0,email:null}}}function ve(e,t,n,s){try{localStorage.setItem("auth_token",e),localStorage.setItem("refresh_token",t),localStorage.setItem("expires_at",String(n)),s&&localStorage.setItem("user_email",s)}catch(i){console.error("Failed to store tokens:",i)}}function fe(){try{localStorage.removeItem("auth_token"),localStorage.removeItem("refresh_token"),localStorage.removeItem("expires_at"),localStorage.removeItem("user_email")}catch{}}async function et(e){let t=await fetch(`${q}/auth/v1/token?grant_type=refresh_token`,{method:"POST",headers:{"Content-Type":"application/json",apikey:me},body:JSON.stringify({refresh_token:e})});if(!t.ok)throw new Error(`Token refresh failed: ${t.status}`);let n=await t.json();return{accessToken:n.access_token,refreshToken:n.refresh_token,expiresAt:Math.floor(Date.now()/1e3)+(n.expires_in||3600),email:n.user?.email}}async function V(){let e=pe();if(!e.accessToken)return null;let t=Math.floor(Date.now()/1e3);return e.expiresAt-t<300?e.refreshToken?o._refreshingToken?o._refreshingToken:(o._refreshingToken=(async()=>{try{let n=await et(e.refreshToken);return ve(n.accessToken,n.refreshToken,n.expiresAt,n.email),n.accessToken}catch{return B(),null}finally{o._refreshingToken=null}})(),o._refreshingToken):(B(),null):e.accessToken}function _(){let e=window.location.hash;if(e&&e!=="#/"&&e!=="#")try{localStorage.setItem("auth_return_route",e)}catch{}let t=window.location.origin+"/app/",n=`${q}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(t)}`;window.location.href=n}function tt(){let e=window.location.hash;if(!e||!e.includes("access_token"))return!1;let t=new URLSearchParams(e.substring(1)),n=t.get("access_token"),s=t.get("refresh_token"),i=parseInt(t.get("expires_in")||"3600",10);if(!n)return!1;let a=Math.floor(Date.now()/1e3)+i;ve(n,s,a,null);let r="#/";try{let c=localStorage.getItem("auth_return_route");c&&(r=c,localStorage.removeItem("auth_return_route"))}catch{}return history.replaceState(null,"",window.location.pathname+r),nt(n),!0}async function nt(e){try{let t=await fetch(`${q}/auth/v1/user`,{headers:{Authorization:`Bearer ${e}`,apikey:me}});if(t.ok){let n=await t.json();n.email&&(localStorage.setItem("user_email",n.email),o.currentUserEmail=n.email)}}catch{}}async function ge(){if(tt())return o.isLoggedIn=!0,!0;if(await V()){let t=pe();return o.currentUserEmail=t.email,o.isLoggedIn=!0,!0}return o.isLoggedIn=!1,!1}function he(){fe(),o.isLoggedIn=!1,o.currentUserId=null,o.currentUserEmail=null,o.currentUserIsAdmin=!1,o.userIsPro=!1,window.location.hash="#/"}function B(){fe(),o.isLoggedIn=!1,o.currentUserId=null,o.currentUserEmail=null,window.location.hash="#/"}var C={},N=null,be=null;function $(e,t){C[e]=t}function st(){let t=(window.location.hash.slice(1)||"/").split("/").filter(Boolean),n=t.length===0?"/":`/${t[0]}`,s=t.slice(1);return{path:n,params:s}}function it(e){return C[e]?{handler:C[e],path:e}:C["/"]?{handler:C["/"],path:"/"}:null}async function ye(){let{path:e,params:t}=st();if(be===e)return;if(N){try{N()}catch{}N=null}let n=document.getElementById("app-root");if(!n)return;let s=it(e);if(!s){n.innerHTML='<div class="page empty-state"><div class="empty-state-icon">404</div><p>Page not found</p></div>';return}be=e,document.querySelectorAll("#bottom-nav .nav-item").forEach(i=>{let r=(i.getAttribute("href")||"").replace("#","")||"/";i.classList.toggle("active",r===e)});try{N=await s.handler(n,t)}catch(i){console.error(`Route error (${e}):`,i),n.innerHTML='<div class="page empty-state"><p>Something went wrong. Please try again.</p></div>'}}function we(){window.addEventListener("hashchange",ye),ye()}var _e=1e4;function xe(e){return`${o.API_BASE_URL}${e.startsWith("/")?e:`/${e}`}`}async function I(e,t={}){let n=await V();if(!n)return B(),null;let s=xe(e),i={"Content-Type":"application/json",Authorization:`Bearer ${n}`,"X-Timezone":Intl.DateTimeFormat().resolvedOptions().timeZone,...t.headers||{}};try{let a=new AbortController,r=setTimeout(()=>a.abort(),_e),c=await fetch(s,{...t,headers:i,signal:a.signal});return clearTimeout(r),c.status===401?(B(),null):c}catch(a){return a.name==="AbortError"?console.warn(`Request timed out: ${e}`):console.warn(`Fetch error: ${e}`,a),null}}async function A(e,t={}){let n=xe(e);try{let s=new AbortController,i=setTimeout(()=>s.abort(),_e),a=await fetch(n,{...t,headers:{"Content-Type":"application/json",...t.headers||{}},signal:s.signal});return clearTimeout(i),a}catch(s){return console.warn(`Anon fetch error: ${e}`,s),null}}async function Y(e,t=3){for(let n=0;n<=t;n++){try{let s=await e();if(s&&s.status!==503)return s}catch{}n<t&&await new Promise(s=>setTimeout(s,1e3*Math.pow(2,n)))}return null}async function x(e){let t=await I(e);return!t||!t.ok?null:t.json()}async function M(e,t){let n=await I(e,{method:"POST",body:JSON.stringify(t)});return!n||!n.ok?null:n.json()}function Ee(e){let t=o._cachedOverviewData;if(!t){e.innerHTML=`
    ...
    <div class="card">
      ...
      </div>`,document.getElementById("login-btn")?.addEventListener("click",_);return}e.innerHTML=`
    <div class="page">
  ...
  `).join("")}}function ke(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}var R="all",rt="recent",ct="all",D=1,L=[],X="vocab";async function $e(e){if(!o.isLoggedIn){e.innerHTML=`
    ...
    </div>`,document.getElementById("hub-tabs")?.addEventListener("click",t=>{let n=t.target.closest(".chip");n&&(X=n.dataset.tab,document.querySelectorAll("#hub-tabs .chip").forEach(s=>s.classList.toggle("active",s.dataset.tab===X)),D=1,L=[],K())}),await K()}async function K(){lt();let e=document.getElementById("hub-content");e&&(X==="vocab"?await Ie(e):await ut(e))}function lt(){let e=document.getElementById("hub-filters");if(!e)return;let t=[{key:"all",label:"All"},{key:"due",label:"Due"},{key:"learning",label:"Learning"},{key:"mastered",label:"Mastered"},{key:"new",label:"New"}];e.innerHTML=t.map(n=>`<button class="chip${n.key===R?" active":""}" data-filter="${n.key}">${n.label}</button>`).join(""),e.onclick=n=>{let s=n.target.closest(".chip");s&&(R=s.dataset.filter,D=1,L=[],e.querySelectorAll(".chip").forEach(i=>i.classList.toggle("active",i.dataset.filter===R)),K())}}async function Ie(e){D===1&&(e.innerHTML='<div class="skeleton skeleton-card"></div>'.repeat(3));let t=await x(`/learning/vocab?page=${D}&per_page=30&sort=${rt}&filter=${R}&category=${ct}&include_sentences=true`);if(!t||!t.vocab){D===1&&(e.innerHTML=`
        <div class="empty-state">
          <div class="empty-state-icon">&#128218;</div>
          <p>No vocabulary yet. Start reading webtoons to build your word list!</p>
        </div>`);return}D===1&&(L=[]),L=[...L,...t.vocab],dt(e);let n=document.getElementById("hub-load-more");n&&(t.has_more?(n.innerHTML='<button class="btn btn-secondary" id="load-more-btn">Load More</button>',document.getElementById("load-more-btn")?.addEventListener("click",()=>{D++,Ie(e)})):n.innerHTML=L.length>0?`<span class="text-muted" style="font-size:var(--font-sm)">${L.length} words</span>`:"")}function dt(e){if(!L.length){e.innerHTML=`
      <div class="empty-state">
        <div class="empty-state-icon">&#128218;</div>
  ...
  `).join("")}function Ae(e){return(e.mastery||"new")==="mastered"?"badge-pro":"badge-free"}function mt(e){try{let s=new Date(e)-new Date,i=Math.round(s/36e5);return i<0?"Now":i<24?`${i}h`:`${Math.round(i/24)}d`}catch{return e}}function O(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}async function Me(e){if(!o.isLoggedIn){e.innerHTML=`
    ...
    </div>`,document.getElementById("logout-btn")?.addEventListener("click",he),document.getElementById("upgrade-btn")?.addEventListener("click",async()=>{let t=await M("/stripe/create_checkout_session",{success_url:window.location.origin+"/app/#/settings",cancel_url:window.location.origin+"/app/#/settings"});t?.url&&(window.location.href=t.url)}),document.getElementById("manage-sub-btn")?.addEventListener("click",async()=>{let t=await M("/stripe/create_portal_session",{return_url:window.location.origin+"/app/#/settings"});t?.url&&(window.location.href=t.url)}),document.getElementById("daily-goal-select")?.addEventListener("change",async t=>{let n=parseInt(t.target.value,10);o.DAILY_WORDS_GOAL=n,await M("/user/daily_goal",{daily_words_goal:n})})}function pt(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}async function He(e,t){let n=t[0];if(!n){e.innerHTML='<div class="page empty-state"><p>No series selected.</p></div>';return}e.innerHTML=`
    <div class="page">
    ...
    `);let u={};if(o.isLoggedIn){let d=await I(`/chapter_access?webtoon_id=${encodeURIComponent(n)}`);if(d?.ok){let p=await d.json();u=p.access||p||{}}}let l=document.getElementById("episode-list");if(!l)return;if(!a.length){l.innerHTML=`
      <div class="empty-state">
        <div class="empty-state-icon">&#128218;</div>
        <p>No episodes available for this series.</p>
      </div>`;return}let m=a.length>0?a[0].episode_id||a[0].id:null;l.innerHTML=a.map(d=>{let p=d.episode_id||d.id,f=p===m||u[p]?.accessible,b=!f&&!o.userIsPro;return`
      <a href="${b?"javascript:void(0)":`#/reader/${n}/${p}`}"
         class="card list-item" style="text-decoration:none;color:inherit;${b?"opacity:0.5;cursor:default":"cursor:pointer"}">
        <div style="flex:1;min-width:0">
          <div style="font-weight:500">${De(d.episode_title||d.title||`Episode ${d.episode_number||p}`)}</div>
          <div class="text-muted" style="font-size:var(--font-xs);margin-top:var(--space-xs)">
            ${d.panel_count?`${d.panel_count} panels`:""}
          </div>
        </div>
        <div>
          ${b?'<span class="badge badge-free">PRO</span>':f?'<span class="text-success" style="font-size:var(--font-sm)">Free</span>':""}
        </div>
      </a>
    `}).join("")}function De(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}function Be(e,t,n){let s=document.createElement("div");s.className="panel-wrapper",s.dataset.panelHash=t.panel_hash;let i=document.createElement("img");return i.className="panel-image",i.src=`${o.API_BASE_URL}${t.image_url}`,i.alt="",i.loading="lazy",i.dataset.panelHash=t.panel_hash,s.appendChild(i),i.onload=()=>{vt(s,t,n)},t.no_text_detected&&s.classList.add("no-text"),e.appendChild(s),s}function vt(e,t,n){if(!t.translations||!t.image_dimensions)return;let s=t.image_dimensions.width,i=t.image_dimensions.height;!s||!i||t.translations.forEach(a=>{if(a.hidden&&!o.currentUserIsAdmin)return;let r=a.translation||{},c=r.original_text||"",u=r.full_translation||"";if(!a.bounding_box)return;let[l,m,d,p]=a.bounding_box.split(",").map(Number),h=l/s*100,f=m/i*100,b=d/s*100,S=p/i*100,g=document.createElement("div");g.className="translation-hit-zone",g.style.position="absolute",g.style.left=`${h}%`,g.style.top=`${f}%`,g.style.width=`${Math.max(b,3)}%`,g.style.height=`${Math.max(S,3)}%`,g.style.minWidth="44px",g.style.minHeight="44px",g.style.cursor="pointer",g.style.zIndex="10",g.dataset.boundingBox=a.bounding_box,g.dataset.originalText=c,g.dataset.translation=u,g.addEventListener("click",Ze=>{Ze.stopPropagation(),ft(e,g,a,h,f,S),n&&n(a,t.panel_hash)}),e.appendChild(g)})}var H=null;function ft(e,t,n,s,i,a){if(H){if(H.remove(),H.dataset.boundingBox===n.bounding_box){H=null;return}H=null}let r=n.translation||{},c=r.original_text||"",u=r.full_translation||"";if(!u||u==="(translation unavailable)")return;let l=document.createElement("div");l.className="translation-overlay",l.dataset.boundingBox=n.bounding_box,l.style.position="absolute",l.style.left=`${Math.max(0,s-2)}%`,l.style.top=`${i+a+1}%`,l.style.zIndex="20",l.innerHTML=`
    <div class="overlay-korean">${Pe(c)}</div>
    <div class="overlay-english">${Pe(u)}</div>
    ${r.vocab&&r.vocab.length>0?`<div class="overlay-vocab-count">${r.vocab.length} word${r.vocab.length>1?"s":""}</div>`:""}
  `;let m=d=>{!l.contains(d.target)&&!t.contains(d.target)&&(l.remove(),H=null,document.removeEventListener("click",m))};setTimeout(()=>document.addEventListener("click",m),0),e.appendChild(l),H=l}function Pe(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}var y=null,Re=null;function J(){return y||(y=document.createElement("div"),y.className="subtitle-bar hidden",y.id="pwa-subtitle-bar",document.body.appendChild(y),y.addEventListener("click",e=>{e.target.closest(".subtitle-vocab-btn")||Z()}),y)}function Oe(e,t){if(y||J(),!e?.translation)return;Re=e;let n=e.translation,s=n.original_text||"",i=n.full_translation||"";y.innerHTML=`
  ...
  `,y.classList.remove("hidden")}function Z(){y&&(y.classList.add("hidden"),Re=null)}function Ce(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}function E(e,t="info"){let n=document.getElementById("toast-container");if(!n)return;let s=document.createElement("div");s.className=`toast ${t}`,s.textContent=e,n.appendChild(s),setTimeout(()=>{s.style.opacity="0",s.style.transition="opacity 0.25s",setTimeout(()=>s.remove(),250)},3e3)}var w=null;function te(){w||(w=document.createElement("div"),w.className="vocab-popover hidden",w.id="pwa-vocab-popover",document.body.appendChild(w),document.addEventListener("click",e=>{w&&!w.contains(e.target)&&!e.target.closest(".subtitle-vocab-btn")&&G()}))}function Ne(e,t){w||te(),!(!e||!e.length)&&(w.innerHTML=`
  ...
  `,w.querySelector(".popover-close")?.addEventListener("click",G),w.classList.remove("hidden"))}function G(){w&&w.classList.add("hidden")}function ee(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}function ze(e){return e?e.replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}async function Ge(e,t){let n=t[0],s=t[1];if(!n||!s){e.innerHTML='<div class="page empty-state"><p>Missing webtoon or episode ID.</p></div>';return}e.innerHTML=`
    <div class="page">
      ...
      <div id="reader-footer" class="text-center mt-xl"></div>
    </div>`,J(),te();let a=await(o.isLoggedIn?I:A)(`/pwa/episode_panels?webtoon_id=${encodeURIComponent(n)}&episode_id=${encodeURIComponent(s)}`);if(!a||!a.ok){let m=document.getElementById("reader-panels");m&&(a?.status===401||a?.status===403?(m.innerHTML=`
          <div class="empty-state">
            <div class="empty-state-icon">&#128274;</div>
            <p class="mb-lg">Log in to read this chapter</p>
            <button class="btn btn-primary" id="reader-login-btn">Continue with Google</button>
          </div>`,document.getElementById("reader-login-btn")?.addEventListener("click",_)):a?.status===404?m.innerHTML=`
          <div class="empty-state">
            <div class="empty-state-icon">&#128218;</div>
          ...
          </div>`:m.innerHTML='<div class="empty-state"><p>Failed to load panels. Please try again.</p></div>');return}let c=(await a.json()).panels||[],u=document.getElementById("reader-panels");if(!u)return;if(u.innerHTML="",!c.length){u.innerHTML=`
      <div class="empty-state">
        <div class="empty-state-icon">&#128218;</div>
        <p>No panels available.</p>
      </div>`;return}c.forEach(m=>{Be(u,m,gt)});let l=document.getElementById("reader-footer");return l&&(l.innerHTML=`
      <div class="card">
    ...
    <div class="exercise-card">
      ...
      <div class="exercise-options" id="exercise-options">
        ${c.map((m,d)=>`
          <button class="exercise-option" data-index="${d}" data-value="${ue(m)}">
            ${k(m)}
          </button>
        `).join("")}
      </div>
    </div>
  `;let l=document.getElementById("exercise-options");l?.addEventListener("click",m=>{let d=m.target.closest(".exercise-option");if(!d||d.disabled)return;let p=d.dataset.value,h=Date.now()-u,f=p===a;l.querySelectorAll(".exercise-option").forEach(b=>{b.disabled=!0,b.dataset.value===a&&b.classList.add("correct"),b===d&&!f&&b.classList.add("incorrect")}),F(e,f,i,a),setTimeout(()=>n(p,a,h),1200)})}function Qe(e,t,n){let s=t.item,i=s.surface_form||s.word||s.vocab_word||"",a=s.meaning||s.definition||"",r=s.romanization||"",c=Date.now();e.innerHTML=`
    <div class="exercise-card">
      <div class="exercise-prompt">
        <div class="exercise-word">${k(i)}</div>
        ${r?`<div class="exercise-romanization">${k(r)}</div>`:""}
  ...
  `;let u=document.getElementById("type-answer"),l=document.getElementById("type-submit");u?.focus();function m(){let d=(u?.value||"").trim();if(!d)return;let p=Date.now()-c,h=Dt(d,a);u.disabled=!0,l.disabled=!0,u.classList.add(h?"correct":"incorrect"),F(e,h,i,a),setTimeout(()=>n(d,a,p),1200)}l?.addEventListener("click",m),u?.addEventListener("keydown",d=>{d.key==="Enter"&&m()})}function Dt(e,t){let n=s=>s.toLowerCase().trim().replace(/[^a-z0-9\s]/g,"");return n(e)===n(t)}function Xe(e,t,n){let s=t.item,i=s.surface_form||s.word||s.vocab_word||"",a=s.meaning||s.definition||"",r=[a,...t.distractors].filter(Boolean);de(r);let c=Date.now();e.innerHTML=`
    <div class="exercise-card">
      <div class="exercise-prompt">
      ...
      <div class="exercise-options" id="exercise-options">
        ${r.map((d,p)=>`
          <button class="exercise-option" data-index="${p}" data-value="${ue(d)}">
            ${k(d)}
          </button>
        `).join("")}
      </div>
    </div>
  `;let u=document.getElementById("listen-btn");function l(){try{let d=new SpeechSynthesisUtterance(i);d.lang="ko-KR",d.rate=.9,speechSynthesis.speak(d)}catch{E("Audio not available","error")}}u?.addEventListener("click",l),setTimeout(l,300);let m=document.getElementById("exercise-options");m?.addEventListener("click",d=>{let p=d.target.closest(".exercise-option");if(!p||p.disabled)return;let h=p.dataset.value,f=Date.now()-c,b=h===a;m.querySelectorAll(".exercise-option").forEach(S=>{S.disabled=!0,S.dataset.value===a&&S.classList.add("correct"),S===p&&!b&&S.classList.add("incorrect")}),F(e,b,i,a),setTimeout(()=>n(h,a,f),1200)})}function Ke(e,t,n){let s=t.item,i=s.pattern_name||s.pattern||"",a=s.pattern||i,r=[a,...t.distractors].filter(Boolean);de(r);let c=Date.now();e.innerHTML=`
    <div class="exercise-card">
      <div class="exercise-prompt">
  ...
  `;let u=document.getElementById("exercise-options");u?.addEventListener("click",l=>{let m=l.target.closest(".exercise-option");if(!m||m.disabled)return;let d=m.dataset.value,p=Date.now()-c,h=d===a;u.querySelectorAll(".exercise-option").forEach(f=>{f.disabled=!0,f.dataset.value===a&&f.classList.add("correct"),f===m&&!h&&f.classList.add("incorrect")}),F(e,h,i,a),setTimeout(()=>n(d,a,p),1200)})}function F(e,t,n,s){let i=e.querySelector(".exercise-feedback");i&&i.remove();let a=document.createElement("div");a.className=`exercise-feedback ${t?"correct":"incorrect"}`,a.innerHTML=t?'<span class="feedback-icon">&#10003;</span> Correct!':`<span class="feedback-icon">&#10007;</span> ${k(s)}`,e.querySelector(".exercise-card")?.appendChild(a)}function de(e){for(let t=e.length-1;t>0;t--){let n=Math.floor(Math.random()*(t+1));[e[t],e[n]]=[e[n],e[t]]}return e}function k(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}function ue(e){return e?e.replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}async function Je(e){if(!o.isLoggedIn){e.innerHTML=`
      <div class="page empty-state">
        <div class="empty-state-icon">&#128218;</div>
        <p class="mb-lg">Log in to practice</p>
        <button class="btn btn-primary" id="login-btn">Continue with Google</button>
      </div>`,document.getElementById("login-btn")?.addEventListener("click",_);return}if(qe()){e.innerHTML='<div class="page" id="practice-container"></div>',W();return}e.innerHTML=`
    <div class="page">
  ...
  `,document.getElementById("exit-practice")?.addEventListener("click",()=>{confirm("Exit practice? Your progress so far is saved.")&&(ce(),window.location.hash="#/learn")});let i=document.getElementById("exercise-area");if(!i)return;let a=(r,c,u)=>{Ve(t.currentIndex,r,c,u),W()};switch(n.type){case"vocab_choice":le(i,n,a);break;case"vocab_type":Qe(i,n,a);break;case"listening":Xe(i,n,a);break;case"grammar_fill":Ke(i,n,a);break;default:le(i,n,a)}}function Ht(e){let t=ce();if(!t){e.innerHTML='<div class="empty-state"><p>No session data.</p></div>';return}let n=t.accuracy,s=Math.round(t.durationMs/1e3),i=Math.floor(s/60),a=s%60;e.innerHTML=`
  ...
  `,document.getElementById("practice-again-btn")?.addEventListener("click",async()=>{let r=await re("blended");if(r.error){E(r.message,"info");return}W()})}async function Pt(){if("serviceWorker"in navigator)try{let e=await navigator.serviceWorker.register("/app/sw.js",{scope:"/app/"});e.addEventListener("updatefound",()=>{let t=e.installing;t&&t.addEventListener("statechange",()=>{t.state==="activated"&&navigator.serviceWorker.controller&&E("Update available. Tap to refresh.","info")})})}catch(e){console.warn("SW registration failed:",e)}}function Bt(){let e=document.getElementById("offline-banner");function t(){o.isOnline=navigator.onLine,e&&e.classList.toggle("hidden",o.isOnline)}window.addEventListener("online",async()=>{t();let n=await Ue();if(n>0){E(`Back online. Syncing ${n} practice results...`,"success");let s=await oe();s>0&&E(`Synced ${s} results`,"success")}else E("Back online","success")}),window.addEventListener("offline",()=>{t()}),t()}function Ct(){let e=document.getElementById("header-auth");e&&(o.isLoggedIn?(e.innerHTML=`
      ...
      <button id="header-login-btn" class="btn btn-primary" style="font-size:var(--font-sm);padding:var(--space-md) var(--space-lg)">
        Log In
      </button>`,document.getElementById("header-login-btn")?.addEventListener("click",_)))}async function Rt(){$("/",Le),$("/catalog",Se),$("/learn",$e),$("/settings",Me),$("/series",He),$("/reader",Ge),$("/practice",Je),await ge(),Ct(),we(),Pt(),Bt(),Fe()}function Ot(e){if(!e)return"";let t=document.createElement("div");return t.textContent=e,t.innerHTML}Rt().catch(e=>{console.error("App init failed:",e)});})();
